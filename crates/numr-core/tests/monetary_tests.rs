...
fn test_currency_formats() {
    ...
    assert_eq!(engine.eval("100 USD").to_string(), "$100");
    assert_eq!(engine.eval("100$").to_string(), "$100");
    assert_eq!(engine.eval("100 dollars").to_string(), "$100");

    // EUR formats
    assert_eq!(engine.eval("€50").to_string(), "€50");
    assert_eq!(engine.eval("50 EUR").to_string(), "€50");
    assert_eq!(engine.eval("50 eur").to_string(), "€50");
    assert_eq!(engine.eval("50 euros").to_string(), "€50");

    // GBP formats
    assert_eq!(engine.eval("£75").to_string(), "£75");
    ...
    assert_eq!(engine.eval("1 BTC").to_string(), "₿1");
    assert_eq!(engine.eval("1 bitcoin").to_string(), "₿1");
}
...
fn test_usd_to_eur_conversion() {
    ...
    assert_eq!(result.to_string(), "€920");
}
...
fn test_usd_to_gbp_conversion() {
    ...
    assert_eq!(result.to_string(), "£79");

    let result = engine.eval("$500 in pounds");
    assert_eq!(result.to_string(), "£395");
}
...
fn test_usd_to_jpy_conversion() {
    ...
    assert_eq!(result.to_string(), "¥14950");

    let result = engine.eval("$10 in jpy");
    assert_eq!(result.to_string(), "¥1495");
}
...
fn test_currency_with_variables() {
    ...
    assert_eq!(result.to_string(), "€4600");
}
...
fn test_currency_percentage_operations() {
    ...
    assert_eq!(result.to_string(), "€450");
}
...
fn test_travel_expense_scenario() {
    ...
    // Individual amounts
    assert_eq!(engine.eval("flight").to_string(), "$800");
    assert_eq!(engine.eval("hotel_paris").to_string(), "€500");
    assert_eq!(engine.eval("hotel_london").to_string(), "£300");
