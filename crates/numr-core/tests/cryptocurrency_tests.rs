...
fn test_btc_arithmetic() {
    ...
    assert_eq!(result.to_string(), "₿0.7");
...
fn test_btc_to_other_currencies() {
    ...
    assert_eq!(result.to_string(), "€55200");

    // Smaller amount
    let result = engine.eval("0.1 BTC in EUR");
    assert_eq!(result.to_string(), "€5520");
}
...
fn test_btc_variables() {
    ...
    assert_eq!(result.to_string(), "$15000");
}
...
fn test_profit_loss_scenario() {
    ...
    assert_eq!(result.to_string(), "$6000");
...
fn test_mixed_crypto_fiat() {
    ...
    assert_eq!(crypto_value.to_string(), "$6000");

    // Can calculate total: $6000 + $5000 = $11000
    let result = engine.eval("$6000 + fiat");
    assert_eq!(result.to_string(), "$11000");
}
